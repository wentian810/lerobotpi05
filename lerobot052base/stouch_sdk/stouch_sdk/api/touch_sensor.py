...
class TouchSensor():
    ...
    def __init__(self, usb_id=None, finger_id=None, cap=None):
        ...
        # [KIMI-MODIFY-START] 添加低通滤波相关初始化 - 用于软物体抓取信号平滑
        # 力信号EMA低通滤波缓存
        self._force_ema = {'fx': 0.0, 'fy': 0.0, 'fz': 0.0}
        self._force_alpha = 0.3  # EMA系数: 越大越灵敏(0-1), 软物体建议0.2-0.4
        
        # 图像时域平滑缓存
        self._last_frame = None
        self._frame_alpha = 0.7  # 当前帧权重, 历史帧权重=1-alpha
        
        # 触觉RGB时域差分缓存(用于增强软物体信号)
        self._last_tactile_rgb = None
        # [KIMI-MODIFY-END]
    ...
    def preprocess_frame(self, frame=None):
        ...
        # [KIMI-MODIFY-START] 添加图像时域平滑 - 减少噪声, 增强软物体弱信号
        if self._last_frame is not None:
            # EMA滤波: 当前帧*alpha + 历史帧*(1-alpha)
            self.preprocessed_frame = cv2.addWeighted(
                self.preprocessed_frame, self._frame_alpha,
                self._last_frame, 1 - self._frame_alpha,
                0
            )
        # 更新缓存
        self._last_frame = self.preprocessed_frame.copy()
        # [KIMI-MODIFY-END]
    ...
    # [KIMI-MODIFY-START] 添加力信号EMA低通滤波方法 - 用于平滑力信号, 减少状态机误判
    def get_total_force_filtered(self, frame=None, pressure_matrix=None, flow_x=None, flow_y=None):
        """
        获取经过EMA低通滤波的总力信号
        
        适用于软物体抓取场景, 减少高频噪声导致的力信号抖动,
        避免状态机(IDLE/CONTACT/SLIDING)频繁切换
        ...
        Returns:
            tuple: (fx_filtered, fy_filtered, fz_filtered)
        """
        # 获取原始力信号
        fx, fy, fz = self.get_total_force(frame, pressure_matrix, flow_x, flow_y)
        
        # EMA滤波: y[n] = alpha * x[n] + (1-alpha) * y[n-1]
        alpha = self._force_alpha
        self._force_ema['fx'] = alpha * fx + (1 - alpha) * self._force_ema['fx']
        self._force_ema['fy'] = alpha * fy + (1 - alpha) * self._force_ema['fy']
        self._force_ema['fz'] = alpha * fz + (1 - alpha) * self._force_ema['fz']
        
        return (self._force_ema['fx'], self._force_ema['fy'], self._force_ema['fz'])
    
    def reset_force_filter(self):
        """重置力滤波器状态 - 在新episode开始时调用"""
        self._force_ema = {'fx': 0.0, 'fy': 0.0, 'fz': 0.0}
        self._last_frame = None
        self._last_tactile_rgb = None
        
    def get_tactile_rgb_enhanced(self, pressure_matrix=None, flow_matrix=None, 
                                  pressure_max=50, flow_max=10, flow_min=-10, 
                                  target_size=None, enhance_for_soft=True):
        """
        增强版触觉RGB生成 - 针对软物体优化
        ...
        Args:
            ...
            flow_min: 光流最小值
            target_size: 目标尺寸
            enhance_for_soft: 是否启用软物体增强
        """
        # 获取基础触觉RGB
        tactile_rgb = self.get_tactile_rgb(
        ...
        if not enhance_for_soft or self._last_tactile_rgb is None:
            self._last_tactile_rgb = tactile_rgb.copy()
            return tactile_rgb
        
        # [软物体增强1] 时域差分: 突出变化区域
        diff = cv2.absdiff(tactile_rgb, self._last_tactile_rgb)
        enhanced = cv2.addWeighted(tactile_rgb, 0.7, diff, 0.3, 0)
        
        # [软物体增强2] CLAHE自适应直方图均衡 - 增强局部对比度
    ...
    def get_touch_status(self, frame=None, contact_threshold=None, release_threshold=None, slide_threshold=None, use_filtered=True):
        """
        检测触摸状态
        ...
        Args:
            ...
            use_filtered: 是否使用滤波后的力信号(默认True, 软物体建议开启)
        ...
        # [KIMI-MODIFY-START] 使用滤波后的力信号, 减少状态机抖动
        if use_filtered:
            fx_total, fy_total, fz_total = self.get_total_force_filtered(frame=frame)
        else:
            fx_total, fy_total, fz_total = self.get_total_force(frame=frame)
        # [KIMI-MODIFY-END]
